...
class ReportField extends DbField
{
    ...
    // Get Avg
    public function getAvg(array $records, bool $skipNull = false): void
    {
        $name = $this->getGroupName();
        $avg = 0;
        if (count($records) > 0) {
    ...
    // Get Count
    public function getCnt(array $records, bool $skipNull = false): void
    {
        $name = $this->getGroupName();
        $cnt = 0;
        if (count($records) > 0) {
