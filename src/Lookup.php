...
class Lookup
{
    ...
    public function __construct(
        ...
        $this->cacheProfile = new QueryCacheProfile(0,  $this->Field->TableName . "." . $this->Field->Name);
     ...
     */
    public function renderViewRow(array $row, ?object $renderer = null): array
    {
        ...
        foreach ($this->DisplayFields as $index => $name) {
            ...
            if ($displayField) {
                ...
                if (!IsEmpty($viewValue) && !($sameTable && $name == $this->Field->Name && !in_array($displayField->DataType, [DataType::DATE, DataType::TIME]) && $displayField->OptionCount == 0)) {
