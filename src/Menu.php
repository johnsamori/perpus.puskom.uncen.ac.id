...
class Menu
{
        ...
        bool $isSidebarItem = false): void
    {
        ...
        // Set item active
        if (!$item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->IsCustomUrl && $item->Url != "" && CurrentUrl() == GetUrl($item->Url) || $item->Source == CurrentPage()->TableName) { // Active, modified by Masino Sinaga, September 13 ,2023 by adding this:  || $item->Source == CurrentPage()->TableName
            $item->Active = true;
        }
    }
    ...
    public function render(): ?array
    {
        ...
        $checkUrl = function ($item) use ($url) {
			...
			if (!$item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->IsCustomUrl && CurrentUrl() == GetUrl($item->Url) || $item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->Source == CurrentPage()->PageObjName) { // Active <-- Modified by Masino Sinaga, September 13, 2023
