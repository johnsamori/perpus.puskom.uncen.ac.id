...
class AdvancedSecurity
{
    ...
    public function validateUser(string $usr, string $pwd = ""): bool
    {
        ...
        if (IsSysAdminUser($user)) {
            $profile->setUserID(AdvancedSecurity::ADMIN_USER_ID)
                ->setUserLevel(AdvancedSecurity::ADMIN_USER_LEVEL_ID);
            $this->loginUser($profile);
    ...
    private function validateEntityUser(PasswordAuthenticatedUserInterface|string $user, string $password = "", bool $checkUsernameOnly = true): bool
    {
        ...
        if (Config("USER_ACTIVATED_FIELD_NAME") && !ConvertToBool($user->get(Config("USER_ACTIVATED_FIELD_NAME")))) {
            return false;
        }
        if ($checkUsernameOnly) {
            return true;
    ...
    public function loadFromStorage(): bool
    {
        ...
        if (!IsApi() && !$this->isSysAdmin() && !$this->canGrant() && count($this->UserLevelIDs) > 0 && Config("USER_LEVEL_HIERARCHY_FIELD") == "''") { // Not grant permission and no hierarchy field
    ...
    public function getRoles(int|string|null $userLevelId = null): array
    {
        ...
        return $this->getRoleNamesFromUserLevel($userLevelId);
    }
    ...
    public function getReachableRoles(int|string|null $userLevelId = null): array
    {
        ...
        foreach ($userLevelSubIds as $id) {
            $roles = array_merge($roles, $this->getRoleNamesFromUserLevel($id));
        }
    ...
    // Get role names from user level
    protected function getRoleNamesFromUserLevel(int|string|null $userLevelId = null): array
    {
        global $USER_ROLES;
