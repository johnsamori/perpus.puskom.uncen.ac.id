...
Type::addType("geometry", "PHPMaker2025\\perpus2025baru\\GeometryType"); // "geometry" type
Type::addType("geography", "PHPMaker2025\\perpus2025baru\\GeographyType"); // "geography" type
Type::addType("hierarchyid", "PHPMaker2025\\perpus2025baru\\HierarchyIdType"); // "hierarchyid" type
Type::addType("bytes", "PHPMaker2025\\perpus2025baru\\BytesType"); // "bytes" type
 ...
 * @return ?object
 */
function &CurrentTable(): ?DbTablebase
{
...
function SearchFilter(string $fldExpression, string $fldOpr, ?string $fldVal, DataType $fldType, string $dbid = "DB"): string
{
    ...
    if (in_array($fldOpr, ["=", "<>", "<", "<=", ">", ">="])) {
        ...
        $filter = GetMultiSearchSqlFilter($fldExpression, "'" . AdjustSql($fldVal) . "'", $fldOpr, $dbid);
 ...
 *
 * @param ?string $uri File system URI, e.g. "s3://bucket", "google.storage://bucket", "azure.blob://container"
 * @return ?Filesystem
 */
function GetFileSystem(?string $uri = null): ?Filesystem
{
...
function GetFilePublicUrl(string $path, Filesystem|string|null $fileSystem = null): string
{
    try {
        ...
        $fileSystem = $fileSystem instanceof Filesystem ? $fileSystem : GetFileSystem($pathInfo["storage"] ?? $fileSystem ?? null);
 ...
 */
function SsoStateStore(): SsoStateStoreInterface
{
    return Container(SsoStateSessionStore::class);
}

/**
 * Get session processor for SAML
 */
function SessionProcessor(): SessionProcessor
{
...
{
    return ($user ?? CurrentUser() ?? Profile()->getUser()) instanceof WindowsUser;
}

// Is access token user (Google/Facebook/Microsoft/Saml)
function IsAccessTokenUser(?UserInterface $user = null): bool
{
    return ($user ?? CurrentUser() ?? Profile()->getUser()) instanceof AccessTokenUser;
}
 ...
 *
 * @param mixed $value Value
 * @param Constraint|array $constraints Constraints
 * @param string|GroupSequence|array $groups Groups
 * @return ConstraintViolationListInterface
 */
function Validate(mixed $value, Constraint|array $constraints = null, string|GroupSequence|array $groups = null): ConstraintViolationListInterface
{
