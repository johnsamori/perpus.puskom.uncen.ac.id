...
class UserProfile implements AdvancedUserInterface, Stringable, JsonSerializable
{
    ...
    {
        if ($this->hasUserName() && $cookie = ReadCookie(COOKIE_USER_PROFILE . base64_encode($this->getUserName()))) {
    ...
    {
        if ($this->hasUserName()) {
            WriteCookie(COOKIE_USER_PROFILE . base64_encode($this->getUserName()), Encrypt((string)$this), time() + Config("USER_PROFILE_COOKIE_EXPIRY_TIME"));
        }
        return $this;
    }
