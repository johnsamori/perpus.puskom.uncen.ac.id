...
<input type="hidden" name="fk_ID" value="<?= HtmlEncode($Page->_UserLevel->getSessionValue()) ?>">
<?php } ?>
...
<?php if ($Page->_UserLevel->getSessionValue() != "") { ?>
...
<input type="hidden" id="x__UserLevel" name="x__UserLevel" value="<?= HtmlEncode($Page->_UserLevel->CurrentValue) ?>" data-hidden="1">
...
<textarea data-table="users" data-field="x_Notes" name="x_Notes" id="x_Notes" cols="50" rows="5" placeholder="<?= HtmlEncode($Page->Notes->getPlaceHolder()) ?>"<?= $Page->Notes->editAttributes() ?> aria-describedby="x_Notes_help"><?= $Page->Notes->EditValue ?></textarea>
