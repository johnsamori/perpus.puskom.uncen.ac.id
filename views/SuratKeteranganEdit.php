...
<form name="fsurat_keteranganedit" id="fsurat_keteranganedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { surat_keterangan: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsurat_keteranganedit;
...
<input type="hidden" data-table="surat_keterangan" data-field="x_Nomor_Surat" data-hidden="1" data-old name="o_Nomor_Surat" id="o_Nomor_Surat" value="<?= HtmlEncode($Page->Nomor_Surat->OldValue ?? $Page->Nomor_Surat->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(["fsurat_keteranganedit", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fsurat_keteranganedit', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fsurat_keteranganedit", "x_Tanggal_Surat", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_surat_keterangan_Nama_Mahasiswa">
    <select
        ...
        data-select2-id="fsurat_keteranganedit_x_Nama_Mahasiswa"
        <?php } ?>
...
<?php if (!$Page->Nama_Mahasiswa->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fsurat_keteranganedit", function() {
    var options = { name: "x_Nama_Mahasiswa", selectId: "fsurat_keteranganedit_x_Nama_Mahasiswa" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsurat_keteranganedit.lists.Nama_Mahasiswa?.lookupOptions.length) {
        options.data = { id: "x_Nama_Mahasiswa", form: "fsurat_keteranganedit" };
    } else {
        options.ajax = { id: "x_Nama_Mahasiswa", form: "fsurat_keteranganedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_surat_keterangan_Pejabat">
    <select
        ...
        data-select2-id="fsurat_keteranganedit_x_Pejabat"
        <?php } ?>
        data-table="surat_keterangan"
...
<script<?= Nonce() ?>>
loadjs.ready("fsurat_keteranganedit", function() {
    var options = { name: "x_Pejabat", selectId: "fsurat_keteranganedit_x_Pejabat" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsurat_keteranganedit.lists.Pejabat?.lookupOptions.length) {
        options.data = { id: "x_Pejabat", form: "fsurat_keteranganedit" };
    } else {
        options.ajax = { id: "x_Pejabat", form: "fsurat_keteranganedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_Arsip_Surat" id= "fa_x_Arsip_Surat" value="<?= (Post("fa_x_Arsip_Surat") == "0") ? "0" : "1" ?>">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsurat_keteranganedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsurat_keteranganedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
