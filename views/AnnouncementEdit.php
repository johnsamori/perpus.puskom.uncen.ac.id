...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Announcement_ID->getDisplayValue($Page->Announcement_ID->EditValue))) ?>"></span>
...
<input type="<?= $Page->Topic->getInputTextType() ?>" name="x_Topic" id="x_Topic" data-table="announcement" data-field="x_Topic" value="<?= $Page->Topic->EditValue ?>" size="50" maxlength="50" placeholder="<?= HtmlEncode($Page->Topic->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->Topic->formatPattern()) ?>"<?= $Page->Topic->editAttributes() ?> aria-describedby="x_Topic_help">
...
<?php if ($Page->_Message->Visible) { // Message ?>
    <div id="r__Message"<?= $Page->_Message->rowAttributes() ?>>
        <label id="elh_announcement__Message" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_Message->caption() ?><?= $Page->_Message->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->_Message->cellAttributes() ?>>
<span id="el_announcement__Message">
<?php $Page->_Message->EditAttrs->appendClass("editor"); ?>
<textarea data-table="announcement" data-field="x__Message" name="x__Message" id="x__Message" cols="50" rows="5" placeholder="<?= HtmlEncode($Page->_Message->getPlaceHolder()) ?>"<?= $Page->_Message->editAttributes() ?> aria-describedby="x__Message_help"><?= $Page->_Message->EditValue ?></textarea>
...
loadjs.ready(["fannouncementedit", "editor"], function() {
    ew.createEditor("fannouncementedit", "x__Message", 50, 5, <?= $Page->_Message->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
