...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        .setEmptyRow(
            function (rowIndex) {
                let fobj = this.getForm(),
                    fields = [["_Username",false],["_UserLevel",false],["CompleteName",false],["Photo",false],["Gender",false],["_Email",false],["Activated",true],["ActiveStatus",true]];
        ...
        .setLists({
            "_UserLevel": <?= $Grid->_UserLevel->toClientList($Grid) ?>,
...
<?php } ?>
<?php if ($Grid->_UserLevel->Visible) { // UserLevel ?>
        <th data-name="_UserLevel" class="<?= $Grid->_UserLevel->headerCellClass() ?>"><div id="elh_users__UserLevel" class="users__UserLevel"><?= $Grid->renderFieldHeader($Grid->_UserLevel) ?></div></th>
<?php } ?>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Grid->_UserID->getDisplayValue($Grid->_UserID->EditValue))) ?>"></span>
...
<?php } ?>
    ...
    <?php } ?>
    <?php if ($Grid->_UserLevel->Visible) { // UserLevel ?>
        <td data-name="_UserLevel"<?= $Grid->_UserLevel->cellAttributes() ?>>
<?php if ($Grid->RowType == RowType::ADD) { // Add record ?>
<?php if ($Grid->_UserLevel->getSessionValue() != "") { ?>
<span<?= $Grid->_UserLevel->viewAttributes() ?>>
<span class="form-control-plaintext"><?= $Grid->_UserLevel->getDisplayValue($Grid->_UserLevel->ViewValue) ?></span></span>
<input type="hidden" id="x<?= $Grid->RowIndex ?>__UserLevel" name="x<?= $Grid->RowIndex ?>__UserLevel" value="<?= HtmlEncode($Grid->_UserLevel->CurrentValue) ?>" data-hidden="1">
<?php } elseif (!$Security->canAccess() && $Security->isLoggedIn()) { // No access permission ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_users__UserLevel" class="el_users__UserLevel">
<span class="form-control-plaintext"><?= $Grid->_UserLevel->getDisplayValue($Grid->_UserLevel->EditValue) ?></span>
</span>
<?php } else { ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_users__UserLevel" class="el_users__UserLevel">
    <select
        id="x<?= $Grid->RowIndex ?>__UserLevel"
        name="x<?= $Grid->RowIndex ?>__UserLevel"
        class="form-select ew-select<?= $Grid->_UserLevel->isInvalidClass() ?>"
        <?php if (!$Grid->_UserLevel->IsNativeSelect) { ?>
        data-select2-id="fusersgrid_x<?= $Grid->RowIndex ?>__UserLevel"
        <?php } ?>
        data-table="users"
        data-field="x__UserLevel"
        data-value-separator="<?= $Grid->_UserLevel->displayValueSeparatorAttribute() ?>"
        data-placeholder="<?= HtmlEncode($Grid->_UserLevel->getPlaceHolder()) ?>"
        <?= $Grid->_UserLevel->editAttributes() ?>>
        <?= $Grid->_UserLevel->selectOptionListHtml("x{$Grid->RowIndex}__UserLevel") ?>
    </select>
    <div class="invalid-feedback"><?= $Grid->_UserLevel->getErrorMessage() ?></div>
<?= $Grid->_UserLevel->Lookup->getParamTag($Grid, "p_x" . $Grid->RowIndex . "__UserLevel") ?>
<?php if (!$Grid->_UserLevel->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fusersgrid", function() {
    var options = { name: "x<?= $Grid->RowIndex ?>__UserLevel", selectId: "fusersgrid_x<?= $Grid->RowIndex ?>__UserLevel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersgrid.lists._UserLevel?.lookupOptions.length) {
        options.data = { id: "x<?= $Grid->RowIndex ?>__UserLevel", form: "fusersgrid" };
    } else {
        options.ajax = { id: "x<?= $Grid->RowIndex ?>__UserLevel", form: "fusersgrid", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
<input type="hidden" data-table="users" data-field="x__UserLevel" data-hidden="1" data-old name="o<?= $Grid->RowIndex ?>__UserLevel" id="o<?= $Grid->RowIndex ?>__UserLevel" value="<?= HtmlEncode($Grid->_UserLevel->OldValue) ?>">
<?php } ?>
<?php if ($Grid->RowType == RowType::EDIT) { // Edit record ?>
<?php if ($Grid->_UserLevel->getSessionValue() != "") { ?>
<span<?= $Grid->_UserLevel->viewAttributes() ?>>
...
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_users__UserLevel" class="el_users__UserLevel">
<span<?= $Grid->_UserLevel->viewAttributes() ?>>
<?= $Grid->_UserLevel->getViewValue() ?></span>
</span>
<?php if ($Grid->isConfirm()) { ?>
<input type="hidden" data-table="users" data-field="x__UserLevel" data-hidden="1" name="fusersgrid$x<?= $Grid->RowIndex ?>__UserLevel" id="fusersgrid$x<?= $Grid->RowIndex ?>__UserLevel" value="<?= HtmlEncode($Grid->_UserLevel->FormValue) ?>">
<input type="hidden" data-table="users" data-field="x__UserLevel" data-hidden="1" data-old name="fusersgrid$o<?= $Grid->RowIndex ?>__UserLevel" id="fusersgrid$o<?= $Grid->RowIndex ?>__UserLevel" value="<?= HtmlEncode($Grid->_UserLevel->OldValue) ?>">
<?php } ?>
<?php } ?>
</td>
    <?php } ?>
...
<input type="<?= $Grid->_Email->getInputTextType() ?>" name="x<?= $Grid->RowIndex ?>__Email" id="x<?= $Grid->RowIndex ?>__Email" data-table="users" data-field="x__Email" value="<?= $Grid->_Email->EditValue ?>" size="50" maxlength="255" placeholder="<?= HtmlEncode($Grid->_Email->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Grid->_Email->formatPattern()) ?>"<?= $Grid->_Email->editAttributes() ?>>
