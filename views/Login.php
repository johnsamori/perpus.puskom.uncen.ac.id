...
<?php
$formAction = Config("USE_TWO_FACTOR_AUTHENTICATION") ? UrlFor("login1fa") : UrlFor("login");
?>
<form name="flogin" id="flogin" class="ew-form ew-login-form" action="<?= $formAction ?>" method="post" novalidate autocomplete="off">
    ...
    <div class="row gx-0">
        <input type="text" name="username" id="username" autocomplete="username" value="<?= HtmlEncode($Page->Username->CurrentValue) ?>" placeholder="<?= HtmlEncode($Language->phrase("Username", true)) ?>"<?= $Page->Username->editAttributes() ?>>
        <div class="invalid-feedback"><?= $Page->Username->getErrorMessage() ?></div>
    </div>
    <?php if (!Config("OTP_ONLY")) { // Disable password checking ?>
    <div class="row gx-0">
        <div class="input-group px-0">
            <input type="password" name="password" id="password" autocomplete="current-password" placeholder="<?= HtmlEncode($Language->phrase("Password", true)) ?>"<?= $Page->Password->editAttributes() ?>>
