...
<span id="el_theuserprofile__UserLevel">
    <select
        ...
        data-select2-id="ftheuserprofileadd_x__UserLevel"
        <?php } ?>
...
loadjs.ready("ftheuserprofileadd", function() {
    var options = { name: "x__UserLevel", selectId: "ftheuserprofileadd_x__UserLevel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftheuserprofileadd.lists._UserLevel?.lookupOptions.length) {
        options.data = { id: "x__UserLevel", form: "ftheuserprofileadd" };
    } else {
        options.ajax = { id: "x__UserLevel", form: "ftheuserprofileadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<textarea data-table="theuserprofile" data-field="x__Profile" name="x__Profile" id="x__Profile" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->_Profile->getPlaceHolder()) ?>"<?= $Page->_Profile->editAttributes() ?> aria-describedby="x__Profile_help"><?= $Page->_Profile->EditValue ?></textarea>
