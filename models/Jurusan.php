...
/**
 * Table class for jurusan
 */
class Jurusan extends DbTable
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "jurusan"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_jurusan
        $this->id_jurusan = new DbField(
            $this, // Table
            'x_id_jurusan', // Variable name
            'id_jurusan', // Name
            '`id_jurusan`', // Expression
            '`id_jurusan`', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_jurusan`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_jurusan->InputTextType = "text";
        $this->id_jurusan->Raw = true;
        $this->id_jurusan->IsAutoIncrement = true; // Autoincrement field
        $this->id_jurusan->IsPrimaryKey = true; // Primary key field
        $this->id_jurusan->Nullable = false; // NOT NULL field
        $this->id_jurusan->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_jurusan->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jurusan";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_jurusan->setDbValue($conn->lastInsertId());
            $row['id_jurusan'] = $this->id_jurusan->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_jurusan']) && !IsEmpty($this->id_jurusan->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_jurusan', $row)) {
                AddFilter($where, QuotedName('id_jurusan', $this->Dbid) . '=' . QuotedValue($row['id_jurusan'], $this->id_jurusan->DataType, $this->Dbid));
            }
        }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_jurusan->CurrentValue : $this->id_jurusan->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_jurusan->CurrentValue = $keys[0];
            } else {
                $this->id_jurusan->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_jurusan', $row) ? $row['id_jurusan'] : null;
        } else {
            $val = !IsEmpty($this->id_jurusan->OldValue) && !$current ? $this->id_jurusan->OldValue : $this->id_jurusan->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_jurusan@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("jurusanlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "jurusanview" => $this->language->phrase("View"),
            "jurusanedit" => $this->language->phrase("Edit"),
            "jurusanadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JurusanView",
            Config("API_ADD_ACTION") => "JurusanAdd",
            Config("API_EDIT_ACTION") => "JurusanEdit",
            Config("API_DELETE_ACTION") => "JurusanDelete",
            Config("API_LIST_ACTION") => "JurusanList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jurusanview", $parm);
        } else {
            $url = $this->keyUrl("jurusanview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jurusanedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jurusanlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jurusanadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jurusanlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jurusandelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_jurusan\":" . VarToJson($this->id_jurusan->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_jurusan->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_jurusan->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_jurusan") ?? Route("id_jurusan")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_jurusan->CurrentValue = $key;
            } else {
                $this->id_jurusan->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // id_jurusan
        $this->id_jurusan->setupEditAttributes();
