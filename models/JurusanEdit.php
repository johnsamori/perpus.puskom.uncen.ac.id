 ...
 * Page class
 */
class JurusanEdit extends Jurusan
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "JurusanEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "jurusanedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_jurusan") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_jurusan->setQueryStringValue($keyValue);
                $this->id_jurusan->setOldValue($this->id_jurusan->QueryStringValue);
            } elseif (Post("id_jurusan") !== null) {
                $this->id_jurusan->setFormValue(Post("id_jurusan"));
                $this->id_jurusan->setOldValue($this->id_jurusan->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_jurusan") ?? Route("id_jurusan")) !== null) {
                    $this->id_jurusan->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("jurusanlist"); // Return to list page
                        return;
                        ...
                        if ($this->id_jurusan->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->id_jurusan->CurrentValue, $this->CurrentRow['id_jurusan'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("jurusanlist"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "jurusanlist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "jurusanlist") {
                            ...
                            $returnUrl = "jurusanlist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'id_jurusan' before field var 'x_id_jurusan'
        $val = $this->hasFormValue("id_jurusan") ? $this->getFormValue("id_jurusan") : $this->getFormValue("x_id_jurusan");
    ...
    public function renderRow(): void
    {
        ...
        // id_jurusan
        $this->id_jurusan->RowCssClass = "row";
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->id_jurusan->Visible && $this->id_jurusan->Required) {
                if (!$this->id_jurusan->IsDetailKey && IsEmpty($this->id_jurusan->FormValue)) {
                    $this->id_jurusan->addErrorMessage(str_replace("%s", $this->id_jurusan->caption(), $this->id_jurusan->RequiredErrorMessage));
                }
            }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("jurusanlist"), "", $this->TableVar, true);
        $pageId = "edit";
