...
/**
 * Table class for pejabat
 */
class Pejabat extends DbTable
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "pejabat"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Nip->Raw = true;
        $this->Nip->IsPrimaryKey = true; // Primary key field
        $this->Nip->Nullable = false; // NOT NULL field
        $this->Nip->Required = true; // Required field
        $this->Nip->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "pejabat";
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        if ($row) {
            if (array_key_exists('Nip', $row)) {
                AddFilter($where, QuotedName('Nip', $this->Dbid) . '=' . QuotedValue($row['Nip'], $this->Nip->DataType, $this->Dbid));
            }
        }
    ...
    {
        $keys = [];
        $val = $current ? $this->Nip->CurrentValue : $this->Nip->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nip->CurrentValue = $keys[0];
            } else {
                $this->Nip->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nip', $row) ? $row['Nip'] : null;
        } else {
            $val = !IsEmpty($this->Nip->OldValue) && !$current ? $this->Nip->OldValue : $this->Nip->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@Nip@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("pejabatlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "pejabatview" => $this->language->phrase("View"),
            "pejabatedit" => $this->language->phrase("Edit"),
            "pejabatadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PejabatView",
            Config("API_ADD_ACTION") => "PejabatAdd",
            Config("API_EDIT_ACTION") => "PejabatEdit",
            Config("API_DELETE_ACTION") => "PejabatDelete",
            Config("API_LIST_ACTION") => "PejabatList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("pejabatview", $parm);
        } else {
            $url = $this->keyUrl("pejabatview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pejabatedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pejabatlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pejabatadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pejabatlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("pejabatdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nip\":" . VarToJson($this->Nip->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Nip->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nip->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nip") ?? Route("Nip")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->Nip->CurrentValue = $key;
            } else {
                $this->Nip->OldValue = $key;
            }
