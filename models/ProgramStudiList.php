 ...
 * Page class
 */
class ProgramStudiList extends ProgramStudi
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "ProgramStudiList";
    ...
    public string $FormName = "fprogram_studilist";

    // CSS class/style
    public string $CurrentPageName = "programstudilist";
    ...
    {
        $this->id_programstudi->setVisibility();
        $this->Nama_Program_Studi->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (program_studi)
        if (!isset($GLOBALS["program_studi"]) || $GLOBALS["program_studi"]::class == PROJECT_NAMESPACE . "program_studi") {
            $GLOBALS["program_studi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "programstudiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "programstudidelete";
        $this->MultiUpdateUrl = "programstudiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'program_studi');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "programstudiview"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit(): void
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_programstudi->Visible = false;
        }
    }
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fprogram_studigrid";
        }
		...
		if (@$_COOKIE['program_studi_searchpanel'] == 'notactive' || @$_COOKIE['program_studi_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['program_studi_searchpanel'] == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fprogram_studisrch");
        }
        $filterList = Concat($filterList, $this->id_programstudi->AdvancedSearch->toJson(), ","); // Field id_programstudi
        $filterList = Concat($filterList, $this->Nama_Program_Studi->AdvancedSearch->toJson(), ","); // Field Nama_Program_Studi
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fprogram_studisrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_programstudi
        $this->id_programstudi->AdvancedSearch->SearchValue = $filter["x_id_programstudi"] ?? "";
        $this->id_programstudi->AdvancedSearch->SearchOperator = $filter["z_id_programstudi"] ?? "";
        $this->id_programstudi->AdvancedSearch->SearchCondition = $filter["v_id_programstudi"] ?? "";
        $this->id_programstudi->AdvancedSearch->SearchValue2 = $filter["y_id_programstudi"] ?? "";
        $this->id_programstudi->AdvancedSearch->SearchOperator2 = $filter["w_id_programstudi"] ?? "";
        $this->id_programstudi->AdvancedSearch->save();

        // Field Nama_Program_Studi
        $this->Nama_Program_Studi->AdvancedSearch->SearchValue = $filter["x_Nama_Program_Studi"] ?? "";
        $this->Nama_Program_Studi->AdvancedSearch->SearchOperator = $filter["z_Nama_Program_Studi"] ?? "";
        $this->Nama_Program_Studi->AdvancedSearch->SearchCondition = $filter["v_Nama_Program_Studi"] ?? "";
        $this->Nama_Program_Studi->AdvancedSearch->SearchValue2 = $filter["y_Nama_Program_Studi"] ?? "";
        $this->Nama_Program_Studi->AdvancedSearch->SearchOperator2 = $filter["w_Nama_Program_Studi"] ?? "";
        $this->Nama_Program_Studi->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->Nama_Program_Studi;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_programstudi); // id_programstudi
            $this->updateSort($this->Nama_Program_Studi); // Nama_Program_Studi
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_programstudi->setSort("");
                $this->Nama_Program_Studi->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"program_studi\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"program_studi\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"program_studi\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprogram_studilist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprogram_studilist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_programstudi->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"program_studi\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" .
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"fprogram_studilist\"" .
            ...
            ($this->InlineDelete ? " data-msg=\"" . HtmlEncode($this->language->phrase("DeleteConfirm")) . "\" data-data='{\"action\":\"delete\"}'" : " data-data='{\"action\":\"show\"}'") .
            ">" . $this->language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $this->security->canDelete();
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_programstudi");
            $this->createColumnOption($option, "Nama_Program_Studi");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fprogram_studisrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fprogram_studisrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fprogram_studilist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function processListAction(): bool
    {
        ...
        if ($filter != "" && $userAction != "") {
            ...
            if ($listAction) {
                ...
                $errmsg = sprintf($this->language->phrase("CustomActionNotFound"), $userAction);
                if (Post("ajax") == $userAction) { // Ajax
                    echo "<p class=\"text-danger\">" . $errmsg . "</p>";
                    return true;
                } else {
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_program_studi", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_program_studi",
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->id_programstudi->setDbValue($row['id_programstudi']);
    ...
    {
        $row = [];
        $row['id_programstudi'] = $this->id_programstudi->DefaultValue;
        $row['Nama_Program_Studi'] = $this->Nama_Program_Studi->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // id_programstudi
            $this->id_programstudi->ViewValue = $this->id_programstudi->CurrentValue;

            // Nama_Program_Studi
            $this->Nama_Program_Studi->ViewValue = $this->Nama_Program_Studi->CurrentValue;

            // id_programstudi
            $this->id_programstudi->HrefValue = "";
            $this->id_programstudi->TooltipValue = "";
    ...
    {
        //    $pageUrl = $this->pageUrl(false); // removed by Masino Sinaga, Sept 27, 2022 ... Printer Friendly (print) using API EXPORT ACTION, too!
		...
		if ($type == "print" || $custom) { // Begin of added Printer Friendly, by Masino Sinaga, September 27, 2022
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['program_studi_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprogram_studisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprogram_studisrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
    ...
    */
    public function exportData(object $doc): void
    {
        ...
        $result = $this->loadResult($this->StartRecord - 1, $this->DisplayRecords <= 0 ? $this->TotalRecords : $this->DisplayRecords);
        if (!$result || !$doc) {
        ...
        $this->pageDataRendering($header);
        $doc->Text .= $header;
        $this->exportDocument($doc, $result, $this->StartRecord, $this->StopRecord, "");
        $result->free();
