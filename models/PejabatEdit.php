 ...
 * Page class
 */
class PejabatEdit extends Pejabat
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "PejabatEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "pejabatedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nip") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nip->setQueryStringValue($keyValue);
                $this->Nip->setOldValue($this->Nip->QueryStringValue);
            } elseif (Post("Nip") !== null) {
                $this->Nip->setFormValue(Post("Nip"));
                $this->Nip->setOldValue($this->Nip->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nip") ?? Route("Nip")) !== null) {
                    $this->Nip->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("pejabatlist"); // Return to list page
                        return;
                        ...
                        if ($this->Nip->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nip->CurrentValue, $this->CurrentRow['Nip'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("pejabatlist"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "pejabatlist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "pejabatlist") {
                            ...
                            $returnUrl = "pejabatlist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        $val = $this->hasFormValue("Nip") ? $this->getFormValue("Nip") : $this->getFormValue("x_Nip");
        ...
        if ($this->hasFormValue("o_Nip")) {
            $this->Nip->setOldValue($this->getFormValue("o_Nip"));
        }

        // Check field name 'Nama' before field var 'x_Nama'
        $val = $this->hasFormValue("Nama") ? $this->getFormValue("Nama") : $this->getFormValue("x_Nama");
    ...
    protected function editRow(): bool
    {
        ...
        if ($this->Nip->CurrentValue != "") {
            $filterChk = "(`Nip` = '" . AdjustSql($this->Nip->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->Nip->CurrentValue, $this->Nip->caption());
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("pejabatlist"), "", $this->TableVar, true);
        $pageId = "edit";
