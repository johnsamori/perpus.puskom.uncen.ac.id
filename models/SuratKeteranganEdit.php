 ...
 * Page class
 */
class SuratKeteranganEdit extends SuratKeterangan
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "SuratKeteranganEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "suratketeranganedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nomor_Surat") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nomor_Surat->setQueryStringValue($keyValue);
                $this->Nomor_Surat->setOldValue($this->Nomor_Surat->QueryStringValue);
            } elseif (Post("Nomor_Surat") !== null) {
                $this->Nomor_Surat->setFormValue(Post("Nomor_Surat"));
                $this->Nomor_Surat->setOldValue($this->Nomor_Surat->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nomor_Surat") ?? Route("Nomor_Surat")) !== null) {
                    ...
                    $this->Nomor_Surat->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("suratketeranganlist"); // Return to list page
                        return;
                        ...
                        if ($this->Nomor_Surat->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nomor_Surat->CurrentValue, $this->CurrentRow['Nomor_Surat'])) {
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        if ($this->hasFormValue("o_Nomor_Surat")) {
            $this->Nomor_Surat->setOldValue($this->getFormValue("o_Nomor_Surat"));
        }
    ...
    protected function editRow(): bool
    {
        ...
        if ($this->Arsip_Surat->Visible && !$this->Arsip_Surat->Upload->KeepFile) {
            if (!IsEmpty($this->Arsip_Surat->Upload->FileName)) {
                ...
                $this->Arsip_Surat->setDbValueDef($newRow, $this->Arsip_Surat->Upload->FileName, $this->Arsip_Surat->ReadOnly);
            }
        }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // Pejabat
        $this->Pejabat->setDbValueDef($newRow, $this->Pejabat->CurrentValue, $this->Pejabat->ReadOnly);

        // Arsip_Surat
        if ($this->Arsip_Surat->Visible && !$this->Arsip_Surat->ReadOnly && !$this->Arsip_Surat->Upload->KeepFile) {
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['Pejabat'])) { // Pejabat
            $this->Pejabat->CurrentValue = $row['Pejabat'];
        }
        if (isset($row['Arsip_Surat'])) { // Arsip_Surat
            $this->Arsip_Surat->CurrentValue = $row['Arsip_Surat'];
        }
    }
