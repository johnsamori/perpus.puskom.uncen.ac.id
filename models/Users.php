...
class Users extends DbTable
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->_UserLevel->IsForeignKey = true; // Foreign key field
        $this->_UserLevel->setSelectMultiple(false); // Select one
        $this->_UserLevel->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->_UserLevel->PleaseSelectText = $this->language->phrase("PleaseSelect"); // "PleaseSelect" text
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->_UserLevel->Lookup = new Lookup($this->_UserLevel, 'userlevels', false, 'ID', ["Name","","",""], '', "", [], [], [], [], [], [], false, '', '', "`Name`");
                break;
            case "id-ID":
                $this->_UserLevel->Lookup = new Lookup($this->_UserLevel, 'userlevels', false, 'ID', ["Name","","",""], '', "", [], [], [], [], [], [], false, '', '', "`Name`");
                break;
    ...
    public function getMasterFilterFromSession(): string
    {
        ...
        if ($this->getCurrentMasterTable() == "userlevels") {
            ...
            if ($this->_UserLevel->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->ID, $this->_UserLevel->getSessionValue(), $masterTable->ID->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession(): string
    {
        ...
        if ($this->getCurrentMasterTable() == "userlevels") {
            $masterTable = Container("userlevels");
            if ($this->_UserLevel->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->_UserLevel, $this->_UserLevel->getSessionValue(), $masterTable->ID->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter(DbTableBase $masterTable, array $keys): string
    {
        ...
        switch ($masterTable->TableVar) {
            case "userlevels":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->ID, $keys["UserLevel"], $this->_UserLevel->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter(DbTableBase $masterTable): string
    {
        switch ($masterTable->TableVar) {
            case "userlevels":
                return GetKeyFilter($this->_UserLevel, $masterTable->ID->DbValue, $masterTable->ID->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    public function deleteUploadedFiles(array $row)
    {
        ...
        foreach ($oldFiles as $oldFile) {
            if (FileExists($this->Photo->uploadPath() . $oldFile)) {
                DeleteFile($this->Photo->uploadPath() . $oldFile);
            }
        }
        $oldFiles = IsEmpty($row['Avatar']) ? [] : [$row['Avatar']];
        foreach ($oldFiles as $oldFile) {
            if (FileExists($this->Avatar->uploadPath() . $oldFile)) {
                DeleteFile($this->Avatar->uploadPath() . $oldFile);
            }
        }
    }
    ...
    public function addMasterUrl(string $url): string
    {
        if ($this->getCurrentMasterTable() == "userlevels" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            ...
            $url .= "&" . GetForeignKeyUrl("fk_ID", $this->_UserLevel->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    public function renderEditRow()
    {
        ...
        if ($this->_UserLevel->getSessionValue() != "") {
            $this->_UserLevel->CurrentValue = GetForeignKeyValue($this->_UserLevel->getSessionValue());
            if ($this->security->canAdmin()) { // System admin
                $curVal = strval($this->_UserLevel->CurrentValue);
                if ($curVal != "") {
                    $this->_UserLevel->ViewValue = $this->_UserLevel->lookupCacheOption($curVal);
        ...
        $this->CompleteName->EditValue = $this->CompleteName->CurrentValue;
    ...
    public function getUserIDFilter(mixed $userId): string
    {
        ...
        if (count($this->security->UserLevelIDs) > 0) {
            ...
            $userLevelUserIdFilter = $userIdExpression . ' IN (SELECT ' . $userIdExpression . ' FROM ' . "users" . ' WHERE ' . $userLevelExpression . ' IN (' . implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->security->UserLevelIDs) . '))';
