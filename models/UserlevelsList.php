...
// Is modal
		...
		if (@$_COOKIE['userlevels_searchpanel'] == 'notactive' || @$_COOKIE['userlevels_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['userlevels_searchpanel'] == 'active') {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"fuserlevelslist\"" .
...
// Load basic search values
        ...
        if ($this->RowType == RowType::VIEW) {
                ...
                if ($this->Hierarchy->ViewValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        foreach ($rswrk as $row) {
                            $arwrk = $this->Hierarchy->Lookup->renderViewRow($row);
                            $this->Hierarchy->ViewValue->add($this->Hierarchy->displayValue($arwrk));
                        }
                    } else {
            ...
            if ($curVal != "") {
                ...
                if ($this->Level_Origin->ViewValue === null) { // Lookup from database
                    ...
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->Level_Origin->Lookup->renderViewRow($rswrk[0]);
                        $this->Level_Origin->ViewValue = $this->Level_Origin->displayValue($arwrk);
                    } else {
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['userlevels_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
