...
// Is modal
        ...
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "userlevels") {
            ...
            $masterRow = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
		...
		if (@$_COOKIE['users_searchpanel'] == 'notactive' || @$_COOKIE['users_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['users_searchpanel'] == 'active') {
    ...
    public function showFilterList(): void
    {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->_UserLevel, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->_UserLevel->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function setupOtherOptions(): void
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"fuserslist\"" .
...
// Load basic search values
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->security->canAdmin()) { // System admin
                $this->_UserLevel->EditValue = $this->language->phrase("PasswordMask");
            } else {
                ...
                if ($this->_UserLevel->AdvancedSearch->ViewValue !== null) { // Load from cache
                    ...
                    if ($curVal == "") {
                        ...
                        $filterWrk = SearchFilter($this->_UserLevel->Lookup->getTable()->Fields["ID"]->searchExpression(), "=", $this->_UserLevel->AdvancedSearch->SearchValue, $this->_UserLevel->Lookup->getTable()->Fields["ID"]->searchDataType(), "DB");
                    }
            ...
            if (!$this->CompleteName->Raw) {
                $this->CompleteName->AdvancedSearch->SearchValue = HtmlDecode($this->CompleteName->AdvancedSearch->SearchValue);
            }
            $this->CompleteName->EditValue = HtmlEncode($this->CompleteName->AdvancedSearch->SearchValue);
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['users_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
    ...
    */
    public function exportData(object $doc): void
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "userlevels") {
            $userlevels = Container("UserlevelsList");
            $masterResult = $userlevels->loadRs($this->DbMasterFilter); // Load master result
            if ($masterResult) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($userlevels);
                    $userlevels->exportDocument($doc, $masterResult);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
