...
// Is modal
		...
		if (@$_COOKIE['help_searchpanel'] == 'notactive' || @$_COOKIE['help_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['help_searchpanel'] == 'active') {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($this->language->phrase("DeleteSelectedLink")) . "\" form=\"fhelplist\"" .
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['help_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
    ...
    */
    public function exportData(object $doc): void
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "help_categories") {
            $help_categories = Container("HelpCategoriesList");
            $masterResult = $help_categories->loadRs($this->DbMasterFilter); // Load master result
            if ($masterResult) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($help_categories);
                    $help_categories->exportDocument($doc, $masterResult);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
