...
/**
 * Table class for program_studi
 */
class ProgramStudi extends DbTable
{
    ...
    // Fields
    public DbField $id_programstudi;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "program_studi"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_programstudi
        $this->id_programstudi = new DbField(
            $this, // Table
            'x_id_programstudi', // Variable name
            'id_programstudi', // Name
            '`id_programstudi`', // Expression
            '`id_programstudi`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_programstudi`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_programstudi->InputTextType = "text";
        $this->id_programstudi->Raw = true;
        $this->id_programstudi->IsAutoIncrement = true; // Autoincrement field
        $this->id_programstudi->IsPrimaryKey = true; // Primary key field
        $this->id_programstudi->Nullable = false; // NOT NULL field
        $this->id_programstudi->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_programstudi->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_programstudi'] = &$this->id_programstudi;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "program_studi";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_programstudi->setDbValue($conn->lastInsertId());
            $row['id_programstudi'] = $this->id_programstudi->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_programstudi']) && !IsEmpty($this->id_programstudi->CurrentValue)) {
                $row['id_programstudi'] = $this->id_programstudi->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_programstudi', $row)) {
                AddFilter($where, QuotedName('id_programstudi', $this->Dbid) . '=' . QuotedValue($row['id_programstudi'], $this->id_programstudi->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_programstudi->DbValue = $row['id_programstudi'];
    ...
    {
        return "`id_programstudi` = @id_programstudi@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_programstudi->CurrentValue : $this->id_programstudi->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_programstudi->CurrentValue = $keys[0];
            } else {
                $this->id_programstudi->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_programstudi', $row) ? $row['id_programstudi'] : null;
        } else {
            $val = !IsEmpty($this->id_programstudi->OldValue) && !$current ? $this->id_programstudi->OldValue : $this->id_programstudi->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_programstudi@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("programstudilist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "programstudiview" => $this->language->phrase("View"),
            "programstudiedit" => $this->language->phrase("Edit"),
            "programstudiadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ProgramStudiView",
            Config("API_ADD_ACTION") => "ProgramStudiAdd",
            Config("API_EDIT_ACTION") => "ProgramStudiEdit",
            Config("API_DELETE_ACTION") => "ProgramStudiDelete",
            Config("API_LIST_ACTION") => "ProgramStudiList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("programstudiview", $parm);
        } else {
            $url = $this->keyUrl("programstudiview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("programstudiedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("programstudilist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("programstudiadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("programstudilist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("programstudidelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_programstudi\":" . VarToJson($this->id_programstudi->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl(string $url, string $parm = ""): string
    {
        if ($this->id_programstudi->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_programstudi->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_programstudi") ?? Route("id_programstudi")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_programstudi->CurrentValue = $key;
            } else {
                $this->id_programstudi->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_programstudi);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_programstudi);
