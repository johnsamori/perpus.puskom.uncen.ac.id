...
/**
    ...
    */
    public function exportData(object $doc, array $keys): void
    {
        ...
        // Export detail records (users)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("users", explode(",", $this->getCurrentDetailTable()))) {
            $users = Container("UsersList");
            $rsdetail = $users->loadRs($users->getDetailFilterFromSession(), $users->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($users);
                    $users->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("users", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->_UserLevel->IsDetailKey = true;
                    $detailPageObj->_UserLevel->CurrentValue = $this->ID->CurrentValue;
                    $detailPageObj->_UserLevel->setSessionValue($detailPageObj->_UserLevel->CurrentValue);
                }
            }
        }
    }
