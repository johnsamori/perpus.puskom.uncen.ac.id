 ...
 * Page class
 */
class VsrtketList extends Vsrtket
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "VsrtketList";
    ...
    public string $FormName = "fvsrtketlist";

    // CSS class/style
    public string $CurrentPageName = "vsrtketlist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (vsrtket)
        if (!isset($GLOBALS["vsrtket"]) || $GLOBALS["vsrtket"]::class == PROJECT_NAMESPACE . "vsrtket") {
            $GLOBALS["vsrtket"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "vsrtketadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "vsrtketdelete";
        $this->MultiUpdateUrl = "vsrtketupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'vsrtket');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "vsrtketview"); // If View page, no primary button
                } else { // List page
...
// Is modal
		...
		if (@$_COOKIE['vsrtket_searchpanel'] == 'notactive' || @$_COOKIE['vsrtket_searchpanel'] == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['vsrtket_searchpanel'] == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fvsrtketsrch");
        }
        ...
        $filterList = Concat($filterList, $this->No_Reg_Anggota->AdvancedSearch->toJson(), ","); // Field No_Reg_Anggota
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fvsrtketsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field No_Reg_Anggota
        $this->No_Reg_Anggota->AdvancedSearch->SearchValue = $filter["x_No_Reg_Anggota"] ?? "";
        $this->No_Reg_Anggota->AdvancedSearch->SearchOperator = $filter["z_No_Reg_Anggota"] ?? "";
        $this->No_Reg_Anggota->AdvancedSearch->SearchCondition = $filter["v_No_Reg_Anggota"] ?? "";
        $this->No_Reg_Anggota->AdvancedSearch->SearchValue2 = $filter["y_No_Reg_Anggota"] ?? "";
        $this->No_Reg_Anggota->AdvancedSearch->SearchOperator2 = $filter["w_No_Reg_Anggota"] ?? "";
        $this->No_Reg_Anggota->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        $searchFlds[] = &$this->No_Reg_Anggota;
        $searchFlds[] = &$this->Jenjang;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->No_Reg_Anggota); // No_Reg_Anggota
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fvsrtketlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fvsrtketlist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "No_Reg_Anggota");
            $this->createColumnOption($option, "Jenjang");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fvsrtketsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fvsrtketsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fvsrtketlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_vsrtket", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (@$_COOKIE['vsrtket_searchpanel'] == 'notactive' || @$_COOKIE['orders_searchpanel'] == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fvsrtketsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fvsrtketsrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
