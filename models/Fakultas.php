 ...
 */
class Fakultas extends DbTable
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // id_fakulttas
        $this->id_fakulttas = new DbField(
            $this, // Table
            'x_id_fakulttas', // Variable name
            'id_fakulttas', // Name
            '`id_fakulttas`', // Expression
            '`id_fakulttas`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_fakulttas`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_fakulttas->InputTextType = "text";
        $this->id_fakulttas->Raw = true;
        $this->id_fakulttas->IsAutoIncrement = true; // Autoincrement field
        $this->id_fakulttas->IsPrimaryKey = true; // Primary key field
        $this->id_fakulttas->Nullable = false; // NOT NULL field
        $this->id_fakulttas->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_fakulttas->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_fakulttas'] = &$this->id_fakulttas;
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_fakulttas->setDbValue($conn->lastInsertId());
            $row['id_fakulttas'] = $this->id_fakulttas->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_fakulttas']) && !IsEmpty($this->id_fakulttas->CurrentValue)) {
                $row['id_fakulttas'] = $this->id_fakulttas->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_fakulttas', $row)) {
                AddFilter($where, QuotedName('id_fakulttas', $this->Dbid) . '=' . QuotedValue($row['id_fakulttas'], $this->id_fakulttas->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_fakulttas->DbValue = $row['id_fakulttas'];
    ...
    {
        return "`id_fakulttas` = @id_fakulttas@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_fakulttas->CurrentValue : $this->id_fakulttas->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_fakulttas->CurrentValue = $keys[0];
            } else {
                $this->id_fakulttas->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_fakulttas', $row) ? $row['id_fakulttas'] : null;
        } else {
            $val = !IsEmpty($this->id_fakulttas->OldValue) && !$current ? $this->id_fakulttas->OldValue : $this->id_fakulttas->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_fakulttas@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "\"id_fakulttas\":" . VarToJson($this->id_fakulttas->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_fakulttas->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_fakulttas->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_fakulttas") ?? Route("id_fakulttas")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_fakulttas->CurrentValue = $key;
            } else {
                $this->id_fakulttas->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // id_fakulttas
        $this->id_fakulttas->setupEditAttributes();
        $this->id_fakulttas->EditValue = $this->id_fakulttas->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_fakulttas);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_fakulttas);
