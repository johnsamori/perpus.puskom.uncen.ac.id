...
// Use layout
    ...
    public function emptyRow(): bool
    {
        ...
        if (
            $this->hasFormValue("x__UserLevel")
            && $this->hasFormValue("o__UserLevel")
            && $this->_UserLevel->CurrentValue != $this->_UserLevel->DefaultValue
            && !($this->_UserLevel->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->_UserLevel->CurrentValue == $this->_UserLevel->getSessionValue())
        ) {
            return false;
        }
        if (
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        if ($this->hasFormValue("o__UserLevel")) {
            $this->_UserLevel->setOldValue($this->getFormValue("o__UserLevel"));
        }
            ...
            if ($this->_UserLevel->getSessionValue() != "") {
                ...
                if ($this->_UserLevel->ViewValue !== null) { // Load from cache
                    ...
                    if ($curVal == "") {
                        ...
                        $filterWrk = SearchFilter($this->_UserLevel->Lookup->getTable()->Fields["ID"]->searchExpression(), "=", $this->_UserLevel->CurrentValue, $this->_UserLevel->Lookup->getTable()->Fields["ID"]->searchDataType(), "DB");
                    }
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->_UserLevel->Visible && $this->_UserLevel->Required) {
                if ($this->security->canAdmin() && !$this->_UserLevel->IsDetailKey && IsEmpty($this->_UserLevel->FormValue)) {
    ...
    protected function addRow(?array $oldRow = null): bool
    {
        ...
        if ($this->getCurrentMasterTable() == "userlevels") {
            $this->_UserLevel->Visible = true; // Need to insert foreign key
            $this->_UserLevel->CurrentValue = $this->_UserLevel->getSessionValue();
        }
